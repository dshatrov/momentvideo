...
mt_throws AsyncIoResult
TcpConnection::recvFrom (Memory      const mem,
                         Size      * const ret_nread,
...
TcpConnection::getLocalAddress (IpAddress * const mt_nonnull ret_addr)
{
    ...
    int const res = getsockname (fd, (struct sockaddr*) &saddr, &saddr_len);
    if (res == -1) {
        int const err = errno;
        logE_ (_this_func, "getsockname() failed: ", errnoString (err));
...
TcpConnection::open ()
{
    ...
    {
      // Note that TCP_QUICKACK may have no real effect here. From man 7 tcp:
      ...
      // enter/leave quickack mode depending on internal protocol processing and
      // factors such as delayed ack timeouts occurring and data transfer."
