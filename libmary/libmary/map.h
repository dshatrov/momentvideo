...
#ifndef LIBMARY__MAP__H__
#define LIBMARY__MAP__H__
...
public:
	...
	Entry (typename AvlTreeBase<T>::Node * const avl_tree_node)
...
public:
    ...
    template <class C>
    typename MapBase<T>::Entry lookupValue (C const &value) const
    {
	return typename MapBase<T>::Entry (avl_tree.lookupValue (value));
    }

    template <class C>
    typename MapBase<T>::Entry lookup (C const &value) const
    {
