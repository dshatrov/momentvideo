                               ...
                               bool const delete_after_tick)
{
    ...
    if (auto_delete && cb.guard_obj) {
        timer->del_sbn = cb.guard_obj->addDeletionCallback (
...
Timers::deleteTimer (TimerKey const mt_nonnull timer_key)
{
    ...
    if (timer->del_sbn) {
        Ref<Object> const code_ref = timer->timer_cb.getWeakRef().getRef ();
        if (!code_ref) {
...
Timers::getSleepTime_microseconds ()
{
    ...
    Time const result = chain->nearest_time - cur_time;
    logD (timers, _func, ": nearest: ", chain->nearest_time, ", cur: ", cur_time, ", delta: ", result);
...
Timers::Timers (EmbedContainer * const embed_container)
