...
// This check overlaps with lastref_cnt.
        ...
        // Note that we *must* unlock 'shadow_mutex' before calling getRefPtr()
                ...
                do {
                    if (sbn->weak_peer_obj.isValid()) {
                        if (// Objects are allowed to subscribe for deletion of
                            ...
                            sbn->weak_peer_obj.getUnsafePtr() != this)
                        {
...
// no sense: it'd be an unnecessary compilcation for no good reason.
  ...
  // one has to get a reference to the object, which is not possible at this point.
    ...
    if (!deletion_ctx || tlocal->deletion_ctx == deletion_ctx)
        deleteThis ();
    else
        deletion_ctx->scheduleDeletion (this);
}
...
Object::addDeletionCallback (CbDesc<DeletionCallback> const &cb)
{
    ...
    DEBUG (
        printf ("0x%lx %s: sbn: 0x%lx, guard_obj: 0x%lx\n",
                (unsigned long) this, _func_name, (unsigned long) sbn, (unsigned long) cb.guard_obj);
    )
    sbn->obj = this;

    if (cb.guard_obj && cb.guard_obj != this) {
        sbn->mutual_sbn = cb.guard_obj->addDeletionCallbackNonmutual (
                CbDesc<DeletionCallback> (mutualDeletionCallback, sbn, this));
    }
