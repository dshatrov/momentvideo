                ...
                void        * const _pipe_session)
{
    ...
    Ref<LinePipe> const self = pipe_session->weak_line_pipe.getRef ();
    if (!self)
        return;
...
{
    this->filename = st_grab (new (std::nothrow) String (filename));
...
LinePipe::LinePipe (EmbedContainer * const embed_container)
