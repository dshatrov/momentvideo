...
SelectPollGroup::addPollable (CbDesc<Pollable> const &pollable,
                              bool const /* auto_remove */ /* TODO implement auto_remove */)
{
    ...
    pollable->setFeedback (
	    Cb<Feedback> (&pollable_feedback, pollable_entry, NULL /* guard_obj */),
	    pollable.cb_data);
...
SelectPollGroup::SelectPollGroup (EmbedContainer * const embed_container)
...
SelectPollGroup::~SelectPollGroup ()
{
    ...
    posixClose (trigger_pipe [0], _func, "trigger_pipe[0] ");
    posixClose (trigger_pipe [1], _func, "trigger_pipe[1] ");
}
