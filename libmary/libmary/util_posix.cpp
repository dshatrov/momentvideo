                                       ...
                                       FileStat    * const mt_nonnull ret_stat)
{
	...
	ret_stat->file_type = FileType::Socket;
        ...
        if (logLevelOn_ (LogLevel::Error)) {
            logLock ();
            log_locked__ (LogLevel::Error, _func, "Unknown file type:");
            logHex_locked__ (LogLevel::Error, ConstMemory::forObject (stat_buf->st_mode), _func);
            logUnlock ();
        }
	exc_throw (InternalException, InternalException::BackendMalfunction);
	return Result::Failure;
