...
namespace {
class MomentFileModule : public Object
{
    class PathEntry : public Referenced
    {
      public:
        mt_const MomentFileModule *moment_file_module;

        mt_const String path;
        mt_const String prefix;

        mt_const Ref<MConfig::Varlist> varlist;
        mt_const bool enable_varlist_defaults;
    };

    mt_const Ref<MConfig::Varlist> glob_varlist;
    mt_const bool glob_enable_varlist_defaults = true;
    ...
    mt_iface_end
    ...
    #ifdef MOMENT_CTEMPLATE
      Result momentFile_sendTemplate (HttpRequest      *http_req,
                                      IpAddress         local_addr,
      ...
      Result momentFile_sendMemory (ConstMemory  mem,
                                    Sender      * mt_nonnull sender,
                                    ConstMemory  mime_type);
    #endif

    void momentFile_addPathEntry (PathEntry   *path_entry,
                                  HttpService * mt_nonnull http_service);

    void momentFile_addPath (ConstMemory  path,
                             ConstMemory  prefix,
                             HttpService * mt_nonnull http_service);

    void momentFile_addPathForSection (MConfig::Section *section,
                                       HttpService      * mt_nonnull http_service);

  public:
    mt_const Result init (MomentInstance * mt_nonnull moment_instance);

    MomentFileModule (EmbedContainer * const embed_container)
        : Object (embed_container)
    {}
};
}

HttpService::HttpHandler const MomentFileModule::http_handler = {
    httpRequest,
    NULL /* httpMessageBody */
};

Result
MomentFileModule::httpRequest (HttpRequest                      * const mt_nonnull req,
                               ...
                               void                             * const _path_entry)
{
    ...
    MomentFileModule * const self = path_entry->moment_file_module;
  ...
  // TODO On Linux, we could do a better job with sendfile() or splice().
    ...
    ConstMemory ext;
    {
      ...
      #else
        // Note: valgrind < 3.8.1 reports a false-positive memrchr() error.
    ...
    StRef<String> const filename = makeString (path_entry->path.mem(),
...

            {
                ...
                while (!iter.done()) {
                    ...
                    StRef<String> const tmp_filename =
                            makeString (path_entry->path.mem(),
                                        path_entry->path.len() ? "/" : "", "strings.",
                                        alang->lang,
                                        ".tpl");
                    NativeFile tmp_file (NULL /* embed_container */);
    ...
    for (;;) {
        ...
        self->page_pool->getFillPages (&page_list, ConstMemory (buf, num_read));
        total_sent += num_read;
...
#ifdef MOMENT_CTEMPLATE
namespace {
class SendTemplate_PageRequest : public PageRequestManager::PageRequest
{
    class Section : public PageRequestManager::PageRequestSection
    {
      public:
        ctemplate::TemplateDictionary *dict;

        Section (ctemplate::TemplateDictionary * const mt_nonnull dict)
            : dict (dict)
        {}
    };
  ...
  public:
    mt_iface (PageRequestManager::PageRequest)
      ...
      void addHashVar (PageRequestManager::PageRequestSection * const section,
                       ConstMemory const name,
                       ConstMemory const value)
      {
          ctemplate::TemplateDictionary * const tmp_dict =
                  section ? static_cast <Section*> (section)->dict : dict;
          tmp_dict->SetValue (String (name).cstr(), String (value).cstr());
      }

      StRef<PageRequestManager::PageRequestSection> addSection (PageRequestManager::PageRequestSection * const section,
                                                                ConstMemory const name)
      {
          ctemplate::TemplateDictionary * const tmp_dict =
                  section ? static_cast <Section*> (section)->dict : dict;
          ctemplate::TemplateDictionary * const sub_dict = tmp_dict->AddSectionDictionary (String (name).cstr());
          return st_grab (new (std::nothrow) Section (sub_dict));
      }

      void showSection (PageRequestManager::PageRequestSection * const section,
                        ConstMemory const name)
      {
          ctemplate::TemplateDictionary * const tmp_dict =
                  section ? static_cast <Section*> (section)->dict : dict;
          tmp_dict->ShowSection (String (name).cstr());
      }
                         ...
                         MConfig::Varlist * const varlist)
{
    ...
    if (! mt_throws varlist_parser.parseVarlist (path, varlist))
        logD_ (_func, "Could not parse varlist ", path, ": ", exc->toString());
}

static void setDictValue (ctemplate::TemplateDictionary * const mt_nonnull dict,
                          ConstMemory                     const name,
                          ConstMemory                     const value)
{
    dict->SetValue (ctemplate::TemplateString ((char const *) name.buf(),  name.len()),
                    ctemplate::TemplateString ((char const *) value.buf(), value.len()));
}

static void setTwoDictValues (ctemplate::TemplateDictionary * const mt_nonnull dict,
                              ConstMemory                     const name_one,
                              ConstMemory                     const name_two,
                              ConstMemory                     const value)
{
    setDictValue (dict, name_one, value);
    setDictValue (dict, name_two, value);
}

Result
MomentFileModule::momentFile_sendTemplate (HttpRequest      * const http_req,
                                           IpAddress          const local_addr,
                                           ...
                                           ConstMemory        const stringvars_filename)
{
  ...
  // TODO Fill the dictionary only once for consecutive multi-language .tpl attempts.
    ...
    if (strings_filename.len()) {
        ...
        inc_dict->SetFilename (
                ctemplate::TemplateString ((char const *) strings_filename.mem(),
                                           strings_filename.len()));
    }

    {
        ConstMemory const host = http_req->getHost();

        setDictValue (&dict, "Host", (host.len() ? extractDomainFromUrl (host) : makeString (IpAddress_NoPort (local_addr))->mem()));

        setTwoDictValues (&dict, "ThisRtmpServerAddr", "RtmpAddr",
                          String::mem (moment_server->getConfigManager()->getRtmpAddrStr (host, local_addr)));

        setTwoDictValues (&dict, "ThisRtmptServerAddr", "RtmptAddr",
                          String::mem (moment_server->getConfigManager()->getRtmptAddrStr (host, local_addr)));

        setTwoDictValues (&dict, "ThisHlsServerAddr", "HlsAddr",
                          String::mem (moment_server->getConfigManager()->getHlsAddrStr (host, local_addr)));

        setTwoDictValues (&dict, "ThisRtspServerAddr", "RtspAddr",
                          String::mem (moment_server->getConfigManager()->getRtspAddrStr (host, local_addr)));

        setTwoDictValues (&dict, "ThisHttpServerAddr", "HttpAddr",
                          String::mem (moment_server->getConfigManager()->getHttpAddrStr (host, local_addr)));

        setTwoDictValues (&dict, "ThisAdminServerAddr", "AdminAddr",
                          String::mem (moment_server->getConfigManager()->getAdminAddrStr (host, local_addr)));
    }

    {
        ...
        if (stringvars_filename.len()) {
            MConfig::Varlist varlist (NULL /* embed_container */);
        ...
        if (enable_varlist_defaults) {
            MConfig::Varlist varlist (NULL /* embed_container */);
            loadVarlist (makeString (path_dir, path_dir.len() ? "/" : "", "var.defaults")->mem(), &varlist);
            fillDictionaryFromVarlist (&dict, &varlist, &sect_hash);
        }

        {
            Ref<MConfig::Varlist> const server_varlist = moment_server->getConfigManager()->getDefaultVarlist();
        ...
        while (!iter.done()) {
            ...
            StRef<String> sect_name;
            if (entry->enabled)
                sect_name = makeString (entry->sect_name->mem(), "_ON");
            else
                sect_name = makeString (entry->sect_name->mem(), "_OFF");
  ...
  #if 0
    {
      // MD5 auth test.
        ...
        StRef<String> const timed_text = makeString (((Uint64) getUnixtime() + 1800) / 3600 /* auth timestamp */,
                                                   " ",
                                                   client_text,
                                                   "password");
        ...
        StRef<String> const auth_str = makeString (client_text, "|", Memory::forObject (hash_buf));
...

    {
        ...
        PageRequestManager::PageRequestResult const res =
                moment_server->getPageRequestManager()->processPageRequest (&page_req, full_path);
    ...
    std::string str;
    if (!ctemplate::ExpandTemplate (String (filename).cstr(),
...
Result
MomentFileModule::momentFile_sendMemory (ConstMemory   const mem,
                                         Sender      * const mt_nonnull sender,
                                         ConstMemory   const mime_type)
{
    MOMENT_FILE__HEADERS_DATE;
    ...
    sender->sendPages (page_pool, page_list.first, /*msg_offs=*/ 0, page_list.data_len, true /* do_flush */);

    return Result::Success;
}
#endif // MOMENT_CTEMPLATE

void
MomentFileModule::momentFile_addPathEntry (PathEntry   * const path_entry,
                                           HttpService * const mt_nonnull http_service)
{
    ...
    http_service->addHttpHandler (
            CbDesc<HttpService::HttpHandler> (&http_handler, path_entry, this, path_entry),
            path_entry->prefix.mem());
}

void
MomentFileModule::momentFile_addPath (ConstMemory   const path,
                                      ConstMemory   const prefix,
                                      HttpService * const mt_nonnull http_service)
{
    Ref<PathEntry> const path_entry = grabNew <PathEntry> ();
    path_entry->moment_file_module = this;
    path_entry->path.set (path);
    path_entry->prefix.set (prefix);
    path_entry->varlist = grabNewObject <MConfig::Varlist> ();
...
void
MomentFileModule::momentFile_addPathForSection (MConfig::Section * const section,
                                                HttpService      * const mt_nonnull http_service)
{
    logD_ (_func_);
...
//
mt_const Result
MomentFileModule::init (MomentInstance * const mt_nonnull moment_instance)
{
    logI_ (_func, "Initializing mod_file");

    glob_varlist = grabNewObject <MConfig::Varlist> ();

    moment_server = moment_instance->getMomentServer();
    page_pool = moment_server->getPagePool ();

    Ref<MConfig::Config> const config       = moment_server->getConfigManager()->getConfig();
    Ref<HttpService>     const http_service = moment_server->getHttpManager()->getHttpService();
...
namespace MomentFile {

Result momentFileInit (MomentInstance * const mt_nonnull moment_instance)
{
    Ref<MomentFileModule> const moment_file_module = grabNewObject <MomentFileModule> ();
    if (!moment_file_module->init (moment_instance))
        return Result::Failure;

    moment_instance->addDeletionData (moment_file_module);
    return Result::Success;
}
  ...
  libMary_moduleInit (void * const moment_instance_)
          { return MomentFile::momentFileInit (static_cast <MomentInstance*> (moment_instance_)); }
#endif
