...
void
Playlist::doParsePlaylist_XML (xmlDocPtr     doc,
                               PlaybackItem * const mt_nonnull default_playback_item)
{
	...
	if (equal (cur_node_name, "item")) {
	    ...
	    parseItem_XML (doc, cur_node, default_playback_item, item);
	    if (item->id)
		item_hash.add (item);
	} else {
...
void
Playlist::parseItem_XML (xmlDocPtr     doc,
                         ...
                         Item         * const mt_nonnull item)
{
    ...
    *item->playback_item = *default_playback_item;
    ...
    parseItemAttributes_XML (item_node, item);
}
#endif // MOMENT_LIBXML2

static Result parseTime (ConstMemory   const time_mem,
...
// time without a date.
	...
	*ret_time = 0;
    ...
    StRef<String> const tmp_str = makeString (time_mem);
    char const *str = (char const *) tmp_str->cstr();
	...
	for (j = 0; j < 3; ++j) {
		...
		if (j > 0) {
		    logE_ (_func, "Bad date/time format: ", time_mem);
		    return Result::Failure;
		}
		...
		if (separator != ':' &&
		    separator != '/')
		{
		    logE_ (_func, "Bad separator in date/time value: ", time_mem);
		    return Result::Failure;
		}
		++str;
	    } else
	...
	if (j > 0) {
	    if (separator == '/') {
		if (got_time) {
		    logE_ (_func, "Duplicate time in date/time value: ", time_mem);
		    return Result::Failure;
		}
		got_time = true;
	    ...
	    if (separator == ':') {
		if (got_date) {
		    logE_ (_func, "Duplicate date in date/time value: ", time_mem);
		    return Result::Failure;
		}
		got_date = true;
    ...
    if (*str) {
	logE_ (_func, "Couldn't parse date/time value: ", time_mem);
	return Result::Failure;
    }
	...
	*ret_time = g_date_time_to_unix (date_time);
...

	{
	  // mktime() gives good results.
		...
		logE_ (_func, "Couldn't convert date/time to unixtime: ", time_mem);
		return Result::Failure;
	    }
...
#ifdef MOMENT_LIBXML2
void
Playlist::parseItemAttributes_XML (xmlNodePtr  node,
                                   Item       * const mt_nonnull item)
{
    {
	xmlChar * const id_val = xmlGetProp (node, (xmlChar const *) "id");
	if (id_val)
	    item->id = st_grab (new (std::nothrow) String ((char const *) id_val));
    }
	...
	if (start_val) {
	    ...
	    ConstMemory const start_mem ((Byte const *) start_val, strlen ((char const *) start_val));
	    if (!parseTime (start_mem, &item->start_time))
		item->start_immediate = true;
	} else {
	    item->start_time = 0;
	    item->start_immediate = true;
	...
	if (end_val) {
	    ...
	    ConstMemory const end_mem ((Byte const *) end_val, strlen ((char const *) end_val));
	    if (!parseTime (end_mem, &item->end_time))
		item->got_end_time = false;
	} else {
	    item->end_time = 0;
	    item->got_end_time = false;
				   ...
				   StRef<String> * const ret_err_msg)
{
	...
	if (ret_err_msg)
	    *ret_err_msg = st_grab (new (std::nothrow) String (err_msg_mem));

	xmlResetError (err);
    } else {
	logE_ (_func, "Could not parse playlist", filename.len() ? " file " : "", filename);
	if (ret_err_msg)
	    *ret_err_msg = makeString ("");
    }
}
#endif // MOMENT_LIBXML2
...
Playlist::setSingleChannelRecorder (ConstMemory const channel_name)
{
    Item * const item = new (std::nothrow) Item;
    assert (item);
    item->id = st_grab (new (std::nothrow) String (channel_name));
    item_list.append (item);
}

mt_throws Result
Playlist::parsePlaylistFile_XML (ConstMemory     const filename,
                                 PlaybackItem  * const mt_nonnull default_playback_item,
                                 StRef<String> * const ret_err_msg)
{
  #ifdef MOMENT_LIBXML2
    ...
    doParsePlaylist_XML (doc, default_playback_item);

    xmlFreeDoc (doc);
    return Result::Success;
  #else
    logE_ (_this_func, "libXML2 support is not enabled in this build");
    return Result::Failure;
  #endif // MOMENT_LIBXML2
}

Result
Playlist::parseItem_Conf (MConfig::Section * const mt_nonnull item_section,
                          PlaybackItem     * const mt_nonnull default_playback_item,
                          Item             * const mt_nonnull item)
{
    item->playback_item = grab (new (std::nothrow) PlaybackItem);
    *item->playback_item = *default_playback_item;
    ...
    if (!parseChannelItemConfig (item_section, default_playback_item, item->playback_item))
        return Result::Failure;

    if (MConfig::Option * const id_opt = item_section->getOption ("id")) {
        if (MConfig::Value * const id_val = id_opt->getValue())
            item->id = st_grab (new (std::nothrow) String (id_val->mem()));
    }

    {
        ...
        if (MConfig::Option * const start_opt = item_section->getOption ("start")) {
            if (MConfig::Value * const start_val = start_opt->getValue()) {
                if (!parseTime (start_val->mem(), &item->start_time)) {
                    logE_ (_func, "parseTime (\"", start_val->mem(), "\") failed");
                    return Result::Failure;
                }
...

    {
        ...
        if (MConfig::Option * const end_opt = item_section->getOption ("end")) {
            if (MConfig::Value * const end_val = end_opt->getValue()) {
                if (!parseTime (end_val->mem(), &item->end_time)) {
                    logE_ (_func, "parseTime (\"", end_val->mem(), "\") failed");
                    return Result::Failure;
                }
...

    {
        ...
        if (MConfig::Option * const duration_opt = item_section->getOption ("duration")) {
            if (MConfig::Value * const duration_val = duration_opt->getValue()) {
                if (equal (duration_val->mem(), "full")) {
                    item->duration_default = false;
                    item->duration_full = true;
                    item->duration = (Time) -1;
                } else {
                    item->duration_default = false;
                    item->duration_full = false;
                    if (!parseDuration (duration_val->mem(), &item->duration)) {
                        item->duration_default = true;
                        item->duration_full = false;
                        item->duration = (Time) -1;
                        logE_ (_func, "couldn't parse duration: ", duration_val->mem());
                        return Result::Failure;
                    }
                }
            }
...

    {
        ...
        if (MConfig::Option * const seek_opt = item_section->getOption ("seek")) {
            if (MConfig::Value * const seek_val = seek_opt->getValue()) {
                if (!parseDuration (seek_val->mem(), &item->seek)) {
                    logE_ (_func, "couldn't parse seek: ", seek_val->mem());
                    return Result::Failure;
                }
            }
        }
...
mt_throws Result
Playlist::parsePlaylistFile_Conf (ConstMemory    const filename,
                                  PlaybackItem * const mt_nonnull default_playback_item)
{
    Ref<MConfig::Config> const playlist_config = grab (new (std::nothrow) MConfig::Config (NULL /* embed_container */));

    if (!MConfig::parseConfig (filename, playlist_config)) {
        exc_throw (InternalException, InternalException::BadInput);
        return Result::Failure;
    }

    return parsePlaylist_ConfSection (playlist_config->getRootSection(),
                                      default_playback_item);
}

Result
Playlist::parsePlaylist_ConfSection (MConfig::Section * mt_nonnull section,
                                     PlaybackItem     * mt_nonnull default_playback_item)
{
    ...
    while (!iter.done()) {
        ...
        if (sect_entry->getType() == MConfig::SectionEntry::Type_Section
            && equal (sect_entry->getName(), "item"))
        {
            MConfig::Section * const item_section = static_cast <MConfig::Section*> (sect_entry);

            Item * const item = new (std::nothrow) Item;
            assert (item);
            item_list.append (item);

            if (!parseItem_Conf (item_section, default_playback_item, item)) {
                delete item;
                return Result::Failure;
            }
...
// Unused
mt_throws Result
Playlist::parsePlaylistMem_XML (ConstMemory     const mem,
                                PlaybackItem  * const mt_nonnull default_playback_item,
                                StRef<String> * const ret_err_msg)
{
...
mt_throws Result
Playlist::parsePlaylistMem_Conf (ConstMemory     const mem,
                                 PlaybackItem  * const mt_nonnull default_playback_item,
                                 StRef<String> * const ret_err_msg)
{
  // TODO
...
//    logD_ (_func_);
    ...
    StRef<Vfs> const vfs = Vfs::createDefaultLocalVfs (root);
    StRef<Vfs::VfsDirectory> const dir = vfs->openDirectory (from_dir);
    if (!dir) {
        logE_ (_func, "openDirectory() failed: ", exc->toString());
        return Result::Failure;
    }

    typedef AvlTree< StRef<String>,
    ...
    while (!iter.done()) {
        StRef<String> const &entry_name = iter.next ()->value;

#if 0
// Unnecessary
        FileState file_stat;
        StRef<String> const path = makeString (root, from_dir, "/", entry_name);
        ...
        item->id = st_grab (new (std::nothrow) String (entry_name->mem()));
        item_hash.add (item);

        item->playback_item = grab (new (std::nothrow) PlaybackItem);
        *item->playback_item = *default_playback_item;

        StRef<String> const uri = makeString ("file://", root, from_dir, "/", entry_name->mem());
//        logD_ (_func, "uri: ", uri);
...
Playlist::dump ()
{
    ...
    while (!iter.done()) {
        Item * const item = iter.next ();
        item->playback_item->dump (LogLevel::Debug, _func, "item #", i, ": ");
        ++i;
    }
}
