...
//#error sequence_headers_sent is always 'false'
...
MediaReader::readIndexAndSeek (bool * const mt_nonnull ret_seeked)
{
    *ret_seeked = false;
    ...
    StRef<String> const idx_filename = makeString (cur_filename, ".idx");
    StRef<Vfs::VfsFile> const idx_file = vfs->openFile (idx_filename->mem(),
                                                        0 /* open_flags */,
...
//            Byte * const hdr = page_list.first->getData();
            ...
            if (hdr [0] == 2) {
              // Audio frame
                AudioFrameType const frame_type = toAudioFrameType (hdr [1]);
                logS_ (_func, "ts ", msg_unixtime_ts_nanosec, " ", frame_type);

                bool skip = false;
                if (frame_type == AudioFrameType::AacSequenceHeader) {
                    if (got_aac_seq_hdr
                ...
                if (!skip && frame_type.isAudioData()) {
                    if (read_frame_cb && read_frame_cb->audioFrame) {
                        if (got_aac_seq_hdr && !aac_seq_hdr_sent) {
                            ...
                            AudioMessage msg;
                            msg.dts_nanosec = msg_unixtime_ts_nanosec;
//#warning TODO Handle PTS properly.
                            msg.pts_nanosec = msg.dts_nanosec;
                            ...
                            msg.frame_type = AudioFrameType::AacSequenceHeader;
...
//#warning TODO Handle PTS properly.
            ...
            if (hdr [0] == 1) {
              // Video frame
                VideoFrameType const frame_type = toVideoFrameType (hdr [1]);
                logS_ (_func, "ts ", msg_unixtime_ts_nanosec, " ", frame_type);

                bool skip = false;
                if (frame_type == VideoFrameType::AvcSequenceHeader) {
                    if (got_avc_seq_hdr
...
//#warning TODO Handle PTS properly.
                            ...
                            msg.frame_type = VideoFrameType::AvcSequenceHeader;
...
void
MediaReader::releaseSequenceHeaders_locked ()
{
    if (got_aac_seq_hdr) {
        page_pool->msgUnref (aac_seq_hdr.first);
