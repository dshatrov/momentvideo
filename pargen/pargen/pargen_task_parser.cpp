	...
	if (equal (token, "[")) {
	  // Particular token
		...
		if (equal (token, "]")) {
			...
			cat_token = makeString (cat_token, token);
		    }
		...
		StRef<String> token = getNextToken (token_stream);
            ...
            return Result::Failure;
	{
		...
		equal (token, "="))
	    {
                ...
                exc_throw (ParsingException,
                           fpos, makeString ("Unexpected '", token, "'"));
                return Result::Failure;
	    }
...
// Note: This might be an alternate mechanism for specifying callbacks
	...
	StRef<String> token = getNonwhspToken (token_stream);
	if (token.isNull ())
	    return decl;
                            ...
                            StRef<Declaration_Callbacks> * const mt_nonnull ret_decl_callbacks)
{
	...
	ConstMemory token;
        if (!getNonwhspToken (token_stream, &token))
            return Result::Failure;
	if (token.len() == 0) {
            exc_throw (ParsingException, fpos, makeString ("'}' expected"));
            return Result::Failure;
        }
