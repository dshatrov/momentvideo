	       ...
	       CompilationOptions const * const mt_nonnull opts)
{
    ...
    if (!file->print ("#ifndef ", opts->all_caps_module_name, "__", opts->all_caps_header_name, "_PARGEN__H__\n"
                      "#define ", opts->all_caps_module_name, "__", opts->all_caps_header_name, "_PARGEN__H__\n"
                      "\n"
                      "\n"
	...
	while (!decl_iter.done ()) {
...

		{
			...
			if (!phrase->phrase_name)
			    phrase_name = makeString ("phrase", phrase_index);
			else
    ...
    if (!file->print ("}\n"
                      "\n"
                      "\n"
                      "#endif /* ", opts->all_caps_module_name, "__", opts->all_caps_header_name, "_PARGEN__H__ */\n"
                      "\n"))
    {
        return Result::Failure;
    }
