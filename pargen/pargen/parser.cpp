...
public:
    ...
    // TODO Unused. Positive cache is not implemented.
...
static void
print_whsp (OutputStream * const mt_nonnull out,
...
static void
print_tab (OutputStream * const mt_nonnull out,
	   Size           const nest_level)
{
    print_whsp (out, nest_level * 1);
}
			   ...
			   ParsingStep_Switch * const mt_nonnull step)
{
    ...
    //         b_opt a c
    //
    // This grammar turns out to be left-recursive if b_opt evaluates
    // to an empty sequence. In this case, we get "a: a c", which can't
    // be processed by our left-to-right parser without special treatment.
    //
    ...
    // a match for the first element of corresponding compound grammars.
    //
    ...
    // the remembered one, then remember the non-left-recursive grammar.
    //
    // Note that multiple occurence of references to the parent grammar
