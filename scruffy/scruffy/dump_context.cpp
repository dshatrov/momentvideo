...
namespace {
	...
	StRef<String> getQualifiedName (ConstMemoryDesc const &name)
	{
...
// This is a nested class. We'll deal with such classes later.
				...
				if (!type_desc__class.isNull ()) {
				  ...
				  // This is a nested class declaration.
				    ...
				    StRef<String> name_str = member->name->toString ();
...
#if 0
	    abortIf (class_->name.isNull ());
	    StRef<String> name_str = class_->name->toString ();
	    abortIf (name_str.isNull ());
#endif

	    {
//		StRef<String> qualified_name = getQualifiedName (name_str->getMemoryDesc ());
