	   ...
	   InternalException)
{
  ...
  // Translating all tokens up to a newline.
    ...
    List< StRef<PpItem> > tmp_pp_items;
    {
	...
	for (;;) {
		...
		defined_guard = true;
		tmp_pp_items.append (pp_token);
	    } else {
		if (!defined_guard)
		    translatePpToken (pp_stream, pp_token, &tmp_pp_items);
		else
		    tmp_pp_items.append (pp_token);
	...
	List< StRef<PpItem> >::Iterator pp_item_iter (tmp_pp_items);
	bool defined_guard = false;
    ...
    Ref<PpItemStream> tmp_pp_stream = grab (static_cast <PpItemStream*> (
						new ListPpItemStream (tmp_pp_items.first, FilePosition ())));
    ...
    ppItemsToTokens (&tmp_pp_items, &token_list);
	   ...
	   InternalException)
{
	...
	StRef<UnicharPpItemStream> tmp_pp_stream =
                st_grab (new (std::nothrow) UnicharPpItemStream (tmp_unichar_stream, pp_token_match_func, String::mem (extra_nondigits)));
	   ...
	   ParsingException)
{
	...
	List< StRef<String> >::Element *cur_param = params->first;
	while (cur_param != NULL) {
	    StRef<String> &str = cur_param->data;
	    errf->print ("\"")
		 .print (str)
		 .print ("\"");
	   ...
	   ParsingException)
{
	...
	// TODO
      ...
      #ifndef LIBMARY_NO_EXCEPTIONS
	throw ParsingException (pp_token->fpos,
				makeString ("invalid preprocessing directive #",
                                            pp_directive_name));
      #endif
    }
}
		...
		if (equal (rpl_token->str->mem(), param_el->data->mem())) {
		  ...
		  /* (End of checking for a neighboring sharp or double-sharp) */
			    ...
			    sp_pp_token_type != PpTokenStringLiteral)
			{
			    ...
			    throw ParsingException (
				    arguments_fpos,
                                    makeString ("argument spelling is not a valid character string literal "
...
void
CppPreprocessor::evaluateDoubleSharps (List< StRef<PpItem> > *tmp_pp_items,
				       MacroBan *macro_ban)
    throw (InternalException,
	   ParsingException)
{
    if (tmp_pp_items == NULL)
	return;

    List< StRef<PpItem> >::Element *cur = tmp_pp_items->first,
                                   *prv_pp_token_el = NULL;
    while (cur) {
	...
	if (equal (pp_token->str->mem(), "##")) {
	    ...
	    StRef<String> new_token_str = makeString (prv_pp_token->str, next_pp_token->str);
	    DEBUG (
		errf->print ("Scruffy.CppParser.evaluateDoubleSharps: "
	   ...
	   ParsingException)
{
  ...
  /* (End of initialization) */
    ...
    StRef<PpItemStream> pp_stream =
            st_grab (static_cast <PpItemStream*> (
                    new (std::nothrow) UnicharPpItemStream (unichar_stream, pp_token_match_func, String::mem (extra_nondigits))));
...
void
CppPreprocessor::translationPhase6 ()
{
  // Concatenating adjacent string literals.

    PpItemList::iterator iter (*pp_items);
    while (!iter.done()) {
        PpItemList::Element * const el = iter.next ();
        if (el->next) {
            PpItem * const left = el->data;
            PpItem *right = el->next->data;

            if (right->type == PpItemWhitespace
                && el->next->next)
            {
                right = el->next->next->data;
            } else {
                continue;
            }

            if (left->type  == PpItemPpToken &&
                right->type == PpItemPpToken)
            {
                PpToken * const left_token  = static_cast <PpToken*> (left);
                PpToken * const right_token = static_cast <PpToken*> (right);

                if (left_token->pp_token_type  == PpTokenStringLiteral &&
                    right_token->pp_token_type == PpTokenStringLiteral)
                {
                    if (String::len (left_token->str)  >= 1 &&
                        String::len (right_token->str) >= 1)
                    {
                        // Stripping quotes
                        right_token->str = makeString (left_token->str->mem().region (0, left_token->str->len() - 1),
                                                       right_token->str->mem().region (1));
                        pp_items->remove (el);
                    }
                }
            }
...
CppPreprocessor::CppPreprocessor (File             * const mt_nonnull source_file,
