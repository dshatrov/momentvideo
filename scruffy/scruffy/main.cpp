...
//	return 0;
	...
	file = File::createDefault (filename,
				    ...
				    AccessMode::ReadOnly);
	// TEST
	file = grabNew <CachedFile> (file, 4096 /* page_size */, 256 /* max_pages */);

	Ref<CppPreprocessor> preprocessor = grabNew <CppPreprocessor> (file);
...
// Preprocessor output dump.
    ...
    //			new (std::nothrow) ListPpItemStream (pp_items->first, FilePosition ())));
	    Ref<TokenStream> token_stream =
    //		grab (static_cast <TokenStream*> (
    //			new (std::nothrow) PpItemStreamTokenStream (pp_stream)));
		    grab (new (std::nothrow) ListTokenStream (&token_list));

    //	errf->print ("Parsing...").pendl ();

#if 0
	    Ref<Grammar> grammar = create_test_grammar ();
	    Ref<TestLookupData> lookup_data = grab (new (std::nothrow) TestLookupData);
...
#ifdef SCRUFFY__AST
		    grab (new (std::nothrow) CppParser (ConstMemoryDesc::forString ("AST")));
#else
		    grab (new (std::nothrow) CppParser (ConstMemoryDesc::forString ("default")));
#endif
